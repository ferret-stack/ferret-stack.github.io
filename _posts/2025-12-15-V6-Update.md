...
> **Correction (July 2026).** An off-season audit of the pipeline found that the
> code in production was not running the formula documented below — it was
> running a FiveThirtyEight-style multiplier (`ln(gd+1) × 2.2/(elo_diff×0.001 + 2.2)`)
> instead. True to house rules, we backtested both over four seasons before
> deciding which one survives: the formula documented here won on log-loss and
> Brier score (narrowly — honestly, within noise), so the code now runs exactly
> what this post says. The worked multiplier examples below were also wrong in
> the original post and have been corrected. The full audit also rebuilt the
> historical data and the venue multipliers — the home/away adjustment is now
> measured from the rebuilt data on every run (currently about +10%/−10%, with
> draw multipliers published too) rather than the +11%/−11% quoted above.
...
```python
def margin_of_victory_multiplier(goal_diff: int, elo_diff: int) -> float:
    """
    Margin of victory multiplier.

    goal_diff: winning margin (always positive)
    elo_diff:  winner's ELO minus loser's ELO (negative for an upset)

    - 1-goal wins: multiplier ≈ 1.05 (about standard)
    - Comfortable wins (2-3 goals): multiplier ≈ 1.25-1.4
    - Thrashings (4+ goals): multiplier ≈ 1.5-1.6
    - Upsets get an additional boost
    """
    if goal_diff == 0:
        return 1.0

    base = math.log(goal_diff + 1)

    # Upset bonus: the weaker team winning big means more
    if elo_diff < -50:
...
- A 1-0 win: ~1.05× multiplier (about the standard ELO change)
- A 2-0 win: ~1.25× multiplier (more)
- A 5-0 thrashing: ~1.60× multiplier (significantly more)
- A 5-0 win by an underdog rated 200 points below: ~1.95× multiplier
